package com.example.sonarlintstandalone;
...
class IssueResponseIntegrationTests {
			...
			public void afterConnected(final StompSession session, StompHeaders connectedHeaders) {
				session.subscribe("/topic/greetings", new StompFrameHandler() {
					...
					public void handleFrame(StompHeaders headers, Object payload) {
						IssueResponse issueResponse = (IssueResponse) payload;
						try {
							assertEquals("Hello, Spring!", issueResponse.getContent());
						} catch (Throwable t) {
							failure.set(t);
						} finally {
							session.disconnect();
				...
				try {
					session.send("/app/hello", new IssueParams("Spring"));
				} catch (Throwable t) {
					failure.set(t);
					latch.countDown();
				}
